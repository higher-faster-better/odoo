...
def get_module_test_cases(module):
    ...
    for obj in module.__dict__.values():
        if not isinstance(obj, type) or not issubclass(obj, case.TestCase):
            continue
        ...
        if getattr(test_case_class, 'allow_inherited_tests_method', False):
            # Keep inherited method for specific classes (likely to be removed in future)
            test_cases = inspect.getmembers(test_case_class, callable)
        else:
            # Sort test cases to keep the initial behavior (likely to be removed in future)
...
def get_test_modules(module):
    """Return a list of modules for the addons containing tests to feed into get_module_test_cases()."""
...
def make_suite(module_names, position='at_install'):
    """Creates a test suite for all the tests in the specified modules, filtered by position and tags."""
